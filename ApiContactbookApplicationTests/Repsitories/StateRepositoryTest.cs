...
namespace ApiContactbookApplicationTests.Repsitories
{
    public class StateRepositoryTest
    {
        ...
        [Fact]
        public void GetStateByCountryId_ReturnStates()
        {
            //Arrange
            var id = 1;
            var statesList = new List<State>
            ...
            var target = new StateRepository(mockAbContext.Object);
            //Act
            var actual = target.GetStateByCountryId(id);
            //Assert
            Assert.NotNull(actual);
            mockDbSet.As<IQueryable<State>>().Verify(c => c.Provider, Times.Once);
            mockDbSet.As<IQueryable<State>>().Verify(c => c.Expression, Times.Once);
            mockAbContext.VerifyGet(c => c.States, Times.Once);

        }
        [Fact]
        public void GetStateByCountryId_WhenStatesIsNull()
        {
            //Arrange
            var id = 1;
            var statesList = new List<State>().AsQueryable();
